        ...
        switch (opt)
        {
			...
			case 'h':
			{
				cout << "\tLOCATELANG HELP\n " <<
						...
						"-o Takes as required parameter an <integer> number. Sets the order in which the models will be produced\n" << 
						"   (i.e. the length of the Markov's chain). Default value is 7.\n" <<
						"-s Takes as required parameter a <double> precision number. Sets the smoothing parameter so it can have\n" << 
						"   a generic probability even if the context and character never shows on the training data. Default value is 0.1.\n" <<
						"-w Takes as required parameter <integer> number. Sets the size of the floating window used to compare probabilities\n" 
						"   between models. It will have an influence on the sensitivity of the algorithm. Default value is 3.\n" << 
						"\n" <<
